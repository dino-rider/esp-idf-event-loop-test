...
#ifdef HANDLE_TASK
void wait_task(int iteration)
{
    ESP_LOGI(TAG, "start waiting for %d", iteration);
    vTaskDelay(pdMS_TO_TICKS(5000));
    ESP_LOGI(TAG, "finished waiting %d", iteration);
    vTaskDelete(NULL);
}
#endif
...
static void task_iteration_handler(void* handler_args, esp_event_base_t base, int32_t id, void* event_data)
{
    ...
    #ifdef HANDLE_TASK
    xTaskCreate(wait_task, "wait_task", 2048, (void*)iteration, uxTaskPriorityGet(NULL), NULL);
    #else
    ESP_LOGI(TAG, "start waiting for %d", iteration);
    vTaskDelay(pdMS_TO_TICKS(5000));
    ESP_LOGI(TAG, "finished waiting %d", iteration);
